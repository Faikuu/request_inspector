...
async def generate_token(requested_resource: TokenRequest, db: AsyncSession = Depends(get_db)):
    resource = await verify_resource_password(db, requested_resource.resource_id, requested_resource.password, verify_password)
    if not resource:
        raise HTTPException(status_code=400, detail="Invalid resource ID or password")

    access_token = create_access_token(data={"sub": str(requested_resource.resource_id)}, expires_delta=timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES))
...
async def create_resource(resource: ResourceCreate, db: AsyncSession = Depends(get_db)):
    ...
    new_resource = Resource(password=hashed_password)
    db.add(new_resource)
    await db.commit()
